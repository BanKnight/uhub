...
type GatewayApiKey = NonNullable<
  Awaited<ReturnType<typeof findApiKeyByRawKey>>
>;
...
export async function requireApiKey(
  ...
  if (!apiKey.endpointRules.includes(endpoint)) {
    throw new Response(
      JSON.stringify({ error: "Endpoint is not allowed for this API key" }),
      { status: 403 },
    );
  }
  ...
  if (apiKey.channelIds.length === 0) {
    throw new Response(
      JSON.stringify({ error: "API key has no allowed channels" }),
      { status: 403 },
    );
  }
