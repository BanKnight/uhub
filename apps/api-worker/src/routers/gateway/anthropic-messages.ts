...
function normalizeAnthropicInputBlocks(
  blocks: Array<
    ...
    | {
        type: 'document';
        source: { type: 'base64'; media_type: string } | { type: 'text'; text: string };
      }
  >
) {
  return blocks
...
function createAnthropicStreamResponse(input: {
  ...
  const closeActiveContentBlock = (controller: ReadableStreamDefaultController<Uint8Array>) => {
...
anthropicMessagesRouter.post('/v1/messages', async (c) => {
  ...
  return proxyGatewayRequest({
    ...
    onSuccess: ({ responseBody, traceId: upstreamTraceId }) => {
      ...
      const parsedResponse = chatCompletionsResponseSchema.safeParse(parsedJson);
